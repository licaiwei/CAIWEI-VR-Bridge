...
{
	HmdMatrix34_t mDeviceToAbsoluteTracking;     // 设备的pose
	HmdVector3_t vVelocity;				// 速度 velocity in tracker space in m/s
	HmdVector3_t vAngularVelocity;		// 角速度angular velocity in radians/s (?)
	ETrackingResult eTrackingResult;    // 追踪结果
	bool bPoseIsValid;  				// pose是否是合法的
...
struct Compositor_FrameTiming
{
	...
	* processing that work earlier in the frame. */
	...
	vr::TrackedDevicePose_t m_HmdPose; // pose used by app to render this frame 应用程序渲染这一帧时的姿态
...
public:
	...
	*
	*用控制器的当前状态填充提供的结构，用控制器状态最近更新时提供的姿势填充提供的姿势。
	*如果用户按下或释放按钮时需要一个精确的控制器姿势作为应用程序的输入，请使用此表单。这个函数被弃用，取而代之的是新的IVRInput系统。
	*/ 
